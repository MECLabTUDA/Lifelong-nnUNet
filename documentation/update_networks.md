...
This is a general description on how to update a networks checkpoint, for instance the stored paths in the checkpoint which might be invalid due to a relocating of the models/checkpoint folder or even when using a provided pre-trained (extension) network which was trained on a different machine. This might also be the case if the network is accessible at the path it has been trained on, however the user executing for example the [evaluation script](evaluation.md) using this network has no permission to write to the paths that are specified within this chekcpoint. The misunderstanding that moving the checkpoint to a location where the user has permission without changing the stored paths is a good way to start, however only relocating does not change the fact the stored paths are still the old ones leading to the old directories. In all cases, the internal paths of the checkpoints have to be altered in order for a smooth execution and usage of networks, indifferent whether used for inference, evaluation or anything else.
...
As mentioned previously, the provided might have different base paths that were used for `RESULTS_FOLDER` and other folder like `nnUNet_preprocessed` eg. The print command printing the arguments for the initialization should be sufficient, however cross checking it with the whole checkpoint, ie. ensuring that there are no other paths that have to be changed, would not harm in any way. Knowing which parts the user wants to replace, we can provide multiple use cases showing how to apply this function. As one mioght notice, this function can be used to manipulate any kind of string, so be careful and as thorough a spossible when applying this function *-- and create a backup of the checkpoint(s) when using the function for the first time to avoid a screw up --*.
...
The easiest use cases are the onew where the user uses the `-r` or `-rm` flag. The difference being that `-r` walks through the whole directory solely based on the provided Task IDs (`-t` flag). The `-rm` flag expects a little more arguments. It scans through the whole directory again, but only modifies the checkpoints of the specific trainer.
...
```
Note that the user has to provide the network_trainer although it will not be considered, so this can be set to whatever the user wants since it will not be checked any further as long as something is provided. here we used the wildcart character `_`.
...
Let's assume the user wants to modify the checkpoint of a single model. For this we assume the network was again trained on the Tasks `11`, `12`, and `13` and the user only wants to modify the network trained on `11` and `12` using the [MiB Trainer](mib_training.md) using fold 2 and 4.
```bash
          ~ $ source ~/.bashrc
          ~ $ source activate <your_anaconda_env>

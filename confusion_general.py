...
if anatomy == "brats":
    ...
    all_tasks = list(sorted(filter(lambda x: "Prostate" in x, os.listdir(ROOT))))
elif anatomy == "hippocampus":
    all_tasks = list(sorted(filter(lambda x: x.startswith("Task19"), os.listdir(ROOT))))
elif anatomy == "cardiac":
    all_tasks = ["Task008_mHeartA", "Task009_mHeartB"]
else:
...
plt.savefig(f"plots/confusion/{anatomy}.pdf", bbox_inches="tight")

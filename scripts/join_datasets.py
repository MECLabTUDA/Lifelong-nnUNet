...
def create_joined_dataset(input_datasets: list, output_dataset: str):
    assert output_dataset.startswith("Task"), "Output dataset must start with 'Task'"
    out_task_id = int(output_dataset[len("Task"):len("Task")+3])
    ...
    print(f"nnUNet_plan_and_preprocess -t {out_task_id}")
    os.system(f"nnUNet_plan_and_preprocess -t {out_task_id}")
...
def main():
    ...
    parser.add_argument("-in", "--input", action='store', type=int, nargs="+", default=None,
                    help="Specify a list of task ids that specify the inputs to join. "
                            "Each of these ids must, have a matching folder 'TaskXXX_' in the raw "
                            "data folder.",
                            required=True)
    
    parser.add_argument("-out", help="The new name for the output dataset",
                        required=True)
    
    args = parser.parse_args()
    inputs = [convert_id_to_task_name(i) for i in args.input]
    create_joined_dataset(inputs, args.out)

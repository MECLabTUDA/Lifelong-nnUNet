...
OTHERS_SITE_IDS = [2,3,5,7,8,9,10,11,12,14,15,17,19,22,23]
...
TARGET = os.path.join(nnUNet_raw_data, f"Task{200}_BraTSOthers")
...
#df = df[df["Site No (represents the originating institution)"] == 1]
dfs = [df[df["Site No (represents the originating institution)"] == site_id] for site_id in OTHERS_SITE_IDS]
df = pd.concat(dfs)
...
generate_dataset_json(os.path.join(TARGET, "dataset.json"), os.path.join(TARGET, "imagesTr"), 
                      ...
                      f"ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData-SiteOthers", 
                      dataset_description=f"ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData-SiteOthers", 

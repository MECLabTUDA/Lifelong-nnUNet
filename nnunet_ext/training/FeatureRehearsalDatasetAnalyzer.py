...
from nnunet_ext.training.FeatureRehearsalDataset import FeatureRehearsalDataset, FeatureRehearsalTargetType


class FeatureRehearsalDatasetAnalyzer():
    def __init__(self, dataset: FeatureRehearsalDataset) -> None:
    ...
    def _compute_statistics_with_task_idx(self):
        ...
        self.num_tasks = max(self.dataset.task_idx_array) + 1
        ...
        for sample in self.dataset:
            ...
            if num_foreground_pixel>0:
                self.num_pixel_with_foreground[task_idx] += 1
    ...
    def _compute_statistics_no_task_idx(self):
        ...
        for sample in self.dataset:
            ...
            num_foreground_pixel = np.count_nonzero(target)
            if num_foreground_pixel>0:
                self.num_pixel_with_foreground += 1
    ...
    def __str__(self) -> str:
        ...
        for i in range(self.num_tasks):
            if self.num_samples[i] == 0:
                continue
            s += f"\n Task {i} has {self.num_samples[i]} samples. {round(100 * self.num_samples_with_foreground[i] / self.num_samples[i],2)} % samples contain foreground."
            s += f" {round(100 * self.num_pixel_with_foreground[i] / (self.num_samples[i] * self.num_pixel), 2)} % voxel contain foreground."

...
class nnUNetTrainerFreezedNonLN(nnUNetTrainerMultiHead):
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        r"""Constructor of freezed trainer (except LN layers) for 2D, 3D low resolution and 3D full resolution nnU-Nets.
        """
        ...
        super().__init__(split, task, plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data, deterministic,
                         ...
                         save_csv, del_log, use_vit, vit_type, version, split_gpu, True, ViT_task_specific_ln)
    ...
    def run_training(self, task, output_folder):
        ...
        if not self.freezed and len(self.mh_network.heads) == 1 and task not in self.mh_network.heads:
            ...
            # -- All Heads -- #
            for head in self.mh_network.heads.keys():
                for name, param in self.mh_network.heads[head].named_parameters():
                    # -- If the parameter name is in the list of the head_parameters, ie. param belongs to the head -- #
                    if 'norm' not in name:
                        # -- Set requires_grad accordingly -- #
                        param.requires_grad = False
                    elif 'ViT' not in name:

...
import sklearn
import tqdm
from nnunet_ext.network_architecture.VaePipe import CFullyConnectedVAE4ConditionOnBothFairScale
...
from nnunet_ext.utilities.logger import _EmptybLogger
matplotlib.use("agg")
...
class nnUNetTrainerVAERehearsalBase2Pipe(nnUNetTrainerLoggingMultiHead):
    # -- Trains n tasks sequentially using transfer learning -- #
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, save_interval=5, already_trained_on=None, use_progress=True,
                 identifier=default_plans_identifier, extension='vae_rehearsal_base2_pipe', tasks_list_with_char=None, 
                 #custom args
        ...
        self.VAE_CLASSES = [CFullyConnectedVAE4ConditionOnBothFairScale, CFullyConnectedVAE4ConditionOnBothFairScale]
        ...
        self.log_vae_test = True

    def _maybe_init_logger(self):
        self.wandb_logger = _EmptybLogger(None)
    ...
    def initialize_vae(self, anatomy: str, shape, num_tasks, conditional_dim):
        ...
        #
        #torch.distributed.init_process_group(backend='nccl', rank=0, world_size=1, store=None)
        #self.vae = DistributedDataParallel(self.vae)
        #self.vae = FullyShardedDataParallel(
        #    self.vae,
        #    #fsdp_auto_wrap_policy=default_auto_wrap_policy,
        #    cpu_offloat=CPUOffload(offload_params=True, offload_optimizer=True)
        #)
    ...
    def train_vae_main(self):
        ...
        extracted_features_val_loader = FeatureRehearsalDataLoader(self.extracted_features_dataset_val, batch_size=min(len(self.extracted_features_dataset_val), max_batch_size), 
                                                                   num_workers=8, pin_memory=True, 
                                                deep_supervision_scales=self.deep_supervision_scales, persistent_workers=False,
                                                shuffle=False)
    ...
    def train_vae(self, save_callback, max_num_epochs: int, anatomy: str):
        ...
        if train_using_rehearsal:
            ...
            dataloader = FeatureRehearsalDataLoader(dataset, batch_size=min(len(self.extracted_features_dataset_tr), max_batch_size), 
                                                    ...
                                                    deep_supervision_scales=self.deep_supervision_scales, persistent_workers=False,
                                                    shuffle=True, 
                                                    drop_last=len(dataset)%min(len(self.extracted_features_dataset_tr), max_batch_size) == 1)
            ...
            #if the initial dataset is too small, we need to duplicate it
            if len(self.extracted_features_dataset_tr) < self.feature_rehearsal_dataloader_tr.batch_size * 40:
                dataloader = FeatureRehearsalDataLoader(FeatureRehearsalMultiDataset(self.extracted_features_dataset_tr, 2), self.feature_rehearsal_dataloader_tr.batch_size,
                                                                    num_workers=8, pin_memory=True, 
                                                deep_supervision_scales=self.deep_supervision_scales, persistent_workers=False,
                                                shuffle=True)
    ...
    @torch.no_grad()
    def ood_detection_by_vae_reconstruction(self, d: np.ndarray, task_id: int):
    ...
    @torch.no_grad()
    def ood_detection_by_uncertainty_mse_temperature(self, d: np.ndarray, threshold: float, task_id: int):
        assert hasattr(self, 'vae'), "vae must be loaded before calling this function"
        assert self.network.conv_op == nn.Conv2d

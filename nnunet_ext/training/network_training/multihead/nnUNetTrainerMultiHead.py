...
class nnUNetTrainerMultiHead(nnUNetTrainerV2): # Inherit default trainer class for 2D, 3D low resolution and 3D full resolution U-Net 
    ...
    def run_training(self, task, output_folder, build_folder=True):
        ...
        if self.use_vit and self.ts_msa:
    ...
    def load_checkpoint_ram(self, checkpoint, train=True, network_old=False, checkpoint_old=None):
        ...
        """
        # -- For the last two tasks, create a ViT MSA head if it is a cross_attn version -- #
        if self.use_vit and self.ts_msa:
            for task in self.already_trained_on[str(self.fold)]['tasks_at_time_of_checkpoint'][-2:]:
                if task not in self.network.ViT.blocks.msa_heads:
                    last_head = copy.deepcopy(self.network.ViT.blocks.msa_heads[list(self.network.ViT.blocks.msa_heads.items())[-1][0]])
                    self.network.ViT.blocks.msa_heads[task] = last_head    # NOTE: requires_grad is true here as this previous head was just trained
                    # -- Remove first head if we have more than 2 heads now, as we only need two for cross-attention to save mem -- #
                    if len (self.network.ViT.blocks.msa_heads) > 2:
                        # -- Remove all previous heads until two remain, i.e. the most recent ones -- #
                        while len (self.network.ViT.blocks.msa_heads) != 2:
                            # -- Delete the first, i.e. oldest head -- #
                            del self.network.ViT.blocks.msa_heads[list(self.network.ViT.blocks.msa_heads.items())[0][0]]
                    # -- Freeze last head as it should not be trained during cross-attention! -- #
                    for _, param in self.network.ViT.blocks.msa_heads[list(self.network.ViT.blocks.msa_heads.items())[0][0]].named_parameters():
                        # -- Set requires_grad to False -- #
                        param.requires_grad = False
            # -- Update body and heads of mh_network as well before loading the state dict -- #
            self.mh_network.model = self.network
            self.mh_network.body, head, _, _ = self.mh_network._split_model_recursively_into_body_head(layer_id=0, model=self.network) # Start from root with full model
            for t in self.mh_network.heads.keys():
                self.mh_network.heads[t] = copy.deepcopy(head)

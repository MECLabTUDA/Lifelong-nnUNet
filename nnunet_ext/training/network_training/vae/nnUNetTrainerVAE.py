...
from nnunet_ext.network_architecture.generic_VAE_UNet import GenericVaeUnet
...
class nnUNetTrainerVAE(nnUNetTrainerV2): # Inherit default trainer class for 2D, 3D low resolution and 3D full resolution U-Net 
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, save_interval=5, already_trained_on=None, use_progress=True,
                 identifier=default_plans_identifier, extension='vae', tasks_list_with_char=None, mixed_precision=True,
    ...
    def initialize_network(self):
        ...
        if self.trainer_path is None:
            ...
            if self.use_vit:
                ...
                self.network.__class__ = GenericVaeUnet



                self.mh_network = MultiHead_Module(GenericVaeUnet, self.split, self.tasks_list_with_char[0][0], prev_trainer=self.network,
                                                   input_channels=self.num_input_channels, base_num_features=self.base_num_features,\
                                                   num_classes=self.num_classes, num_pool=len(self.net_num_pool_op_kernel_sizes))
        ...
        assert isinstance(self.trainer_model, (nnUNetTrainerV2, nnUNetTrainerVAE)), "The trainer needs to be nnUNetTrainerV2 or nnUNetTrainerMultiHead.."
        ...
        if self.trainer_model.__class__.__name__ == nnUNetTrainerV2.__name__:   # Important when doing evaluation, since nnUNetTrainerV2 has no mh_network
            self.mh_network = MultiHead_Module(GenericVaeUnet, self.split, self.tasks_list_with_char[0][0], prev_trainer=self.trainer_model.network,
                                               input_channels=self.num_input_channels, base_num_features=self.base_num_features,\
                                               num_classes=self.num_classes, num_pool=len(self.net_num_pool_op_kernel_sizes))
    ...
    def _build_output_path(self, output_folder, meta_data=False):
        ...
        if self.use_vit:
            ...
            if GenericVaeUnet.__name__ != output_folder.split(os.path.sep)[-1] and GenericVaeUnet.__name__ not in output_folder:
                if not meta_data:
                    output_folder = os.path.join(output_folder, GenericVaeUnet.__name__)
                else:   # --> Path were meta data is stored, i.e. already_trained_on file
                    output_folder = os.path.join(output_folder, 'metadata', GenericVaeUnet.__name__)

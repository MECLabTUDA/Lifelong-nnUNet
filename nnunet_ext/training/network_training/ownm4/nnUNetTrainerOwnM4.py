...
HYPERPARAMS = {'pseudo_alpha': float, 'pod_lambda': float, 'pod_scales': int, 'ewc_lambda': float}
...
class nnUNetTrainerOwnM4(nnUNetTrainerMultiHead):
    def __init__(self, split, task, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 ...
                 pod_scales=3, tasks_list_with_char=None, mixed_precision=True, save_csv=True, del_log=False, use_vit=False,
                 vit_type='base', version=1, split_gpu=False, transfer_heads=True, ViT_task_specific_ln=False,
                 do_pod=True, do_LSA=False, do_SPT=False, network=None, use_param_split=False):
        ...
        self.init_args = (split, task, plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                          ...
                          ewc_lambda, pseudo_alpha, pod_lambda, pod_scales, tasks_list_with_char, mixed_precision, save_csv,
    ...
    def initialize(self, training=True, force_load_plans=False, num_epochs=500, prev_trainer_path=None, call_for_eval=False):
        ...
        if prev_trainer_path is not None and not call_for_eval:
            if self.do_pod:
                self.trainer_path = join(os.path.sep, *self.trainer_path.split(os.sep)[:-1], 'pod', "fold_%s" % str(self.fold))
            else:
                self.trainer_path = join(os.path.sep, *self.trainer_path.split(os.sep)[:-1], 'no_pod', "fold_%s" % str(self.fold))
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False, detach=True, no_loss=False):
        ...
        else:   # --> More than one head, ie. trained on more than one task  --> use own method
            if not self.switched:
                # -- Switch to own loss -- #
                self.loss = self.own_loss
                if self.do_pod:
                    # -- We are at a further sequence of training, so we train using the own method -- #

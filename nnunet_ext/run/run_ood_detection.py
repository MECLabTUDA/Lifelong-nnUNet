...
def run_ood_detection():
    ...
    parser.add_argument('--method',
                        help='confidence or vae_reconstruction',
                        required=True,
                        default=None)
    ## eventually add the possibility to use different ood detection strategies
    ...
    for evaluate_on in evaluate_on_tasks:
        ...
        if args.method in ['vae_reconstruction']:
            trainer.load_vae()
        ...
        for preprocessed in preprocessing:
            ...
            if args.method == 'confidence':
                ood_score = trainer.ood_detection_by_confidence(d, args.enable_tta, mixed_precision)
            elif args.method == 'vae_reconstruction':
                ood_score = trainer.ood_detection_by_vae_reconstruction(d, args.enable_tta, mixed_precision)
            else:

# ------------------------------------------------------------------------------
# Other evaluation metrics, from Karol (https://github.com/Karol-G/nnUNet)
# ------------------------------------------------------------------------------
...
def comp_detection_error(tp, fp, tn, fn):
    error = 0.
    if (tp+fn) > 0:
        tpr = tp/(tp+fn)
        error += 0.5*(1-tpr)

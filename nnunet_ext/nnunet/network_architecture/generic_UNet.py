...
# # from nnunet_ext.ignored.highresnet import HighRes3DNet  # # package not found
...
class AltNetwork(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 max_num_features=None, basic_block=ConvDropoutNormNonlin,
                 seg_output_use_bias=False):
        super(AltNetwork, self).__init__()
        self._deep_supervision = deep_supervision
        self.do_ds = deep_supervision
        if dropout_op_kwargs['p']>0:
        ...
        self.num_classes = num_classes
        self.final_nonlin = final_nonlin
        self.network = HighRes3DNet(in_channels=1, out_channels=num_classes, add_dropout_layer=add_dropout_layer)

    def forward(self, x):
        seg_outputs = self.network(x)
        if self._deep_supervision and self.do_ds:

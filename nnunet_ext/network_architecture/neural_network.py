...
# Changes include:
# - adaptation to extract features in the predict_3D method
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          ...
                                          layer_name_for_feature_extraction: str) -> Tuple[np.ndarray, np.ndarray]:
                    ...
                    if feature_dir is not None:
                        ...
                        if isinstance(prediction, tuple): 
                            ...
                            for p in prediction:
                                ...
                                predicted_segmentation = predicted_segmentation.argmax(0)
                                predicted_segmentations.append(predicted_segmentation)
                            predicted_patch = prediction[0]# <- unpack highest resolution of tuple
                            predicted_patch = predicted_patch[0]# <- unpack batch dimension (B,C,D,H,W) -> (C,D,H,W)
                        else:
                        ...
                        predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                            data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                            gaussian_importance_map)[0]# <- unpack batch dimension (B,C,D,H,W) -> (C,D,H,W)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           ...
                                           layer_name_for_feature_extraction: str = None) -> torch.tensor:
